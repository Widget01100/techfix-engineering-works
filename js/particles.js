(function() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    for (let i=0;i<3;i++) particles.push({
      ...
      size: Math.random()*4+2, alpha: 0.8, vx: (Math.random()-0.5)*2, vy: (Math.random()-0.5)*2-1, life: 1
    });
  ...
  function draw() {
    ...
    for (let i=particles.length-1;i>=0;i--) {
      const p = particles[i];
      p.x += p.vx; p.y += p.vy; p.life -= 0.02; p.alpha = p.life * 0.8;
      if (p.life <= 0) { particles.splice(i,1); continue; }
      ctx.beginPath();
      const grad = ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,p.size);
      grad.addColorStop(0, `rgba(250,204,21,${p.alpha})`);
